    ...
    <div class="tm-header-wrapper">
        ...
        <nav class="tm-nav" id="tm-nav">
            <ul>
                ...
                <li class="tm-nav-item active"><a href="${pageContext.request.contextPath}/system/myactivity" class="tm-nav-link">
...
<div id="model"  style="display: none">
    <article class="col-6 col-md-6 tm-post">
        ...
        <button id="deletebutton" type="button" class="layui-btn" id="Deletebutton" onclick="Delete(this, this.value)" value="99999">
            <i class="layui-icon"></i>Sign Out
        </button>
        <div class="d-flex justify-content-between tm-pt-45">
...
<script type="text/javascript">
    ...
    $(function(){
        //var student = "<%=session.getAttribute("userInfo")%>";
        var id = "${userInfo.id}";
        //alert(id)
        $.ajax({
            type: "POST",
            url: "${pageContext.request.contextPath}/activity/selectPersonActivity",
            data: id,
            //dataType: "json",
            ...
            success: function(ajaxResponse){
                ...
                for(var i in data){
                    var ele = document.getElementById("deletebutton")
    ...
    function myFunction(input) {
        //alert(input)
        $("Myinput").val("")
        <%--var student = "<%=session.getAttribute("userInfo").id%>"--%>
        <%--var formdata = {--%>
        <%--    id: student.id,--%>
        <%--    type: input--%>
        <%--}--%>
        $.ajax({
            type: "POST",
            url: "${pageContext.request.contextPath}/activity/getActivityBytype",
            dataType:"json",
            data: input,
            success: function(ajaxResponse){
                ...
                for(var i in data){
                    ...
                    //div = div.replace(/Activity Type/,data[i].type)
                    div = div.replace(/Activity Title/g,data[i].title)
                    //div = div.replace(/Activity Content/g, data[i].content)
                    //div = div.replace(/June 24, 2020/g,data[i].time)
    ...
    function Delete(ele, value){
        ...
        $.ajax({
            url: "${pageContext.request.contextPath}/activity/signOutActivity",
